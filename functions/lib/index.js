...
async function generateInvestigation(question, orgId) {
    ...
    if (!text)
        throw new HttpsError('internal', 'Gemini returned an empty investigation.');
    let parsedJson;
    try {
    ...
    if (!parsed.success)
        throw new HttpsError('internal', 'Gemini returned an incomplete investigation.');
    return parsed.data;
}
...
export const sendWelcomeEmail = onCall({ secrets: [resendApiKey, smtpPassword] }, async (request) => {
    ...
    if (userSnapshot.data()?.welcomeEmailSentAt)
...
export const sendPasswordResetLink = onCall({ secrets: [resendApiKey, smtpPassword] }, async (request) => {
    ...
    if (apiKey) {
        ...
        if (!response.ok)
            throw new HttpsError('internal', 'The password reset email could not be sent.');
    }
    else if (smtpHost.value() && smtpUser.value() && smtpPassword.value()) {
...
export const startInvestigation = onCall({ secrets: [geminiApiKey] }, async (request) => {
    ...
    const db = admin.firestore();
    const orgId = user.token.orgId;
    if (!orgId)
        throw new HttpsError('failed-precondition', 'User is not attached to an organization.');
